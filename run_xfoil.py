...
if run_simulation:
    ...
    for i in range(1,4):
        ...
        for d in data:
            ...
            cl.append(d[1])
            alpha.append(d[0])
        axis[0].plot(alpha,cl,label = names_of_airfoils[-i]+" slope: "+  str(np.format_float_positional(np.polyfit(alpha,cl,1)[0],2)),linewidth=3)
        axis[1].plot(cd,cl,'o',label = names_of_airfoils[-i],linewidth=3)
    ...
    if cd_alpha:
        ...
        for i in range(1,4):
            ...
            for d in data:
                ...
                alpha.append(d[0])
            axis[0].plot(alpha,cd,'o',label = names_of_airfoils[-i],linewidth=3)
            axis[1].plot(alpha,cl_cd,'o',label = names_of_airfoils[-i],linewidth=3)
        axis[0].set_xlabel(u'\u0251'+u'\u00B0',fontsize=16)
