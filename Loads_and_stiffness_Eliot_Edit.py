...
def get_stiffness():
    ...
    def I_xx(y):
        ...
        I = S_stringers * (h / 2) ** 2 * (
                    stringers_top(span, stringertop) + stringers_bottom(span, stringerbot)) + 1 / 12 * t2 * h ** 3 + 2 * w * t1 * (
                    h / 2) ** 2
        return (I)
    ...
    V_total = 2 * t1 * (chord(0) + chord(span_elliot / 2)) / 2 * width * span_elliot / 2 + 2 * t2 * (
            chord(0) + chord(span_elliot / 2)) / 2 * spar_height * span_elliot / 2 + (stringertop + stringerbot)*span_elliot/2 * S_stringers
    mass = V_total * 2 * rho
